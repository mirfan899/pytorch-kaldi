...
def get_val_lst_file_path(out_folder, valid_data, ep, ck, ck_val, fea_name, N_ep_str_format, N_ck_str_format,
                          N_ck_str_format_val):
    def _get_val_lst_file_name(dataset, ep, ck, ck_val, fea_name, N_ep_str_format, N_ck_str_format,
                               N_ck_str_format_val):
        file_name = _get_val_file_name_base(dataset, ep, ck, ck_val, N_ep_str_format, N_ck_str_format,
                                            N_ck_str_format_val)
        file_name += '_'
    ...
    lst_file_name = _get_val_lst_file_name(valid_data, ep, ck, ck_val, fea_name, N_ep_str_format, N_ck_str_format,
                                           N_ck_str_format_val)
    lst_file = out_folder + '/exp_files/' + lst_file_name
    return lst_file


def get_val_info_file_path(out_folder, valid_data, ep, ck, ck_val, N_ep_str_format, N_ck_str_format,
                           N_ck_str_format_val):
    ...
    info_file_name = _get_val_info_file_name(valid_data, ep, ck, ck_val, N_ep_str_format, N_ck_str_format,
                                             N_ck_str_format_val)
    info_file = out_folder + '/exp_files/' + info_file_name
    return info_file


def get_val_cfg_file_path(out_folder, valid_data, ep, ck, ck_val, N_ep_str_format, N_ck_str_format,
                          N_ck_str_format_val):
    ...
    cfg_file_name = _get_val_cfg_file_name(valid_data, ep, ck, ck_val, N_ep_str_format, N_ck_str_format,
                                           N_ck_str_format_val)
                ...
                if do_validation_after_chunk(ck, N_ck_tr, config):
                    for valid_data in valid_data_lst:
                        ...
                        for ck_val in range(N_ck_valid):
                            lst_file = get_val_lst_file_path(out_folder, valid_data, ep, ck, ck_val, None,
                                                             N_ep_str_format, N_ck_str_format, N_ck_str_format_val)
                            info_file = get_val_info_file_path(out_folder, valid_data, ep, ck, ck_val, N_ep_str_format,
                                                               N_ck_str_format, N_ck_str_format_val)
                            config_chunk_file = get_val_cfg_file_path(out_folder, valid_data, ep, ck, ck_val,
                                                                      N_ep_str_format, N_ck_str_format,
                                                                      N_ck_str_format_val)
                            lst_chunk_file.write(config_chunk_file + '\n')
                            write_cfg_chunk(cfg_file, config_chunk_file, cfg_file_proto_chunk, model_files, lst_file,
                                            info_file, 'valid', valid_data, lr, max_seq_length_train_curr, name_data,
                                            ep, ck_val, batch_size_tr_arr[ep], drop_rates)
                    if strtobool(config['batches']['increase_seq_length_train']):
                        if len(max_seq_length_train.split(',')) == 1:
                    ...
                    for dataset_val in valid_data_name:
                        ...
                        for ck_val in range(N_chunks_val):
                            for fea_idx in range(len(fea_names)):
                                ...
                                output_lst_file = get_val_lst_file_path(out_folder, dataset_val, ep, ck, ck_val,
                                                                        fea_names[fea_idx], N_ep_str_format,
                                                                        N_ck_str_format, N_ck_str_format_val)
                                f = open(output_lst_file, 'w')
...
def forward_model_refac01(fea_dict, lab_dict, arch_dict, model, nns, costs, inp, ref, inp_out_dict, max_len_fea,
                          max_len_lab, batch_size, to_do, forward_outs):
    ...
    def _compute_layer_values(inp_out_dict, inp2, inp, inp1, max_len, batch_size, arch_dict, out_name, nns, outs_dict,
                              to_do):
        ...
        if _is_input_feature(inp_out_dict, inp2):
            inp_dnn = _extract_respective_feature_from_input(inp, inp_out_dict, inp2, arch_dict, inp1, max_len,
                                                             batch_size)
    ...
    for line in model:
        ...
        if operation == 'compute':
            outs_dict, do_break = _compute_layer_values(inp_out_dict, inp2, inp, inp1, max_len_fea, batch_size,
                                                        arch_dict, out_name, nns, outs_dict, to_do)
            if do_break:
                break
