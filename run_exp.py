                ...
                for valid_data in valid_data_lst:
                    ...
                    for ck_val in range(N_ck_valid):
                        ...
                        if not (os.path.exists(info_file)):
                            ...
                            next_config_file = cfg_file_list[op_counter]
                            data_name, data_set, data_end_index, fea_dict, lab_dict, arch_dict = run_nn(data_name,
                                                                                                        data_set,
                                                                                                        data_end_index,
                                                                                                        fea_dict,
                                                                                                        lab_dict,
                            ...
                            if not (os.path.exists(info_file)):
                                ...
                                sys.exit(0)
                        op_counter += 1
                    valid_info_lst = sorted(glob.glob(
                        get_val_info_file_path(out_folder, valid_data, ep, ck, None, N_ep_str_format, N_ck_str_format,
                                               N_ck_str_format_val)))
    ...
    for ck in range(N_ck_forward):
        ...
        if not (os.path.exists(info_file)):
            ...
            if _run_forwarding_in_subprocesses(config):
                ...
                use_cuda = strtobool(config['exp']['use_cuda'])
                p = read_next_chunk_into_shared_list_with_subprocess(read_lab_fea, shared_list, config_chunk_file,
                                                                     is_production, output_folder,
                ...
                p = multiprocessing.Process(target=run_nn, kwargs={'data_name': data_name, 'data_set': data_set,
                                                                   'data_end_index': data_end_index,
                                                                   'fea_dict': fea_dict, 'lab_dict': lab_dict,
                                                                   'arch_dict': arch_dict,
                                                                   'cfg_file': config_chunk_file,
                                                                   'processed_first': False, 'next_config_file': None})
                processes.append(p)
                if _max_nr_of_parallel_forwarding_processes(config) != -1 and len(
                        processes) > _max_nr_of_parallel_forwarding_processes(config):
                    processes[0].join()
                    del processes[0]
                p.start()
            else:
